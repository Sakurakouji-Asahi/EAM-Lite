...
更新核对：2026-09-09。本文件集中说明现有 Windows 本机运行与数据操作，发行 ZIP 也可独立使用；源代码开发、AI 需求参考和历史任务不作为普通使用者的前置步骤。各操作仍按当前版本脚本执行。
...
- `D:\资产管理`：开发版，也是唯一日常修改源码的位置；
- `D:\资产管理-正式版`：正式版，只接收经过确认的开发提交，不在这里直接改代码。

两个目录的程序代码可以处于同一版本，但运行数据必须分开。开发版和正式版分别使用独立的
数据库、附件、密钥、备份阶段、容器、端口和环境标识；“同步代码”绝不等于复制开发数据。
...
开发内容验收并提交后，在 `D:\资产管理` 双击 `同步开发版到正式版.cmd`。脚本依次执行：

1. 检查开发版和正式版均无未提交文件，且正式版可以直接快进到开发提交；
2. 启动并检查当前正式版；
3. 要求输入迁移密码，创建并验证更新前数据库与附件一体化备份；
4. 停止正式版，只同步已提交的程序代码；
5. 执行迁移、静态文件、启动和健康检查，并记录旧、新 commit 与备份信息。

任一检查、备份、快进或启动失败都会停止流程；脚本不会用开发数据库、附件或密钥覆盖正式版。
...
已单独配置开发运行目录时，以该目录为准；迁移运行目录应完整保留原有密钥文件。
...
日常将本机开发成果发布到正式版时，在开发版目录双击 `同步开发版到正式版.cmd`。

从 GitHub 正式 Release 或 `origin/main` 获取外部正式更新时，在正式版目录双击
`更新EAM-Lite.cmd`。
...
若要在开发电脑先打印、搬到正式服务器后继续使用同一批标签，须先确定长期不变的公司内网
HTTPS 域名。由网络管理员保证手机能解析并信任该地址，开发验收时将该域名转到开发环境，
迁移验收后再转到正式服务器。开发环境启动前，在 Windows 用户环境变量
`EAM_DEV_DURABLE_QR_BASE_URL` 中填写该域名根地址，例如 `https://eam.company.lan`。
启动脚本会验证它是无端口、无额外路径的 HTTPS 域名，并把它用于新打印的二维码。
域名设置本身不会创建 DNS、HTTPS 证书或网络代理；打印前用实际手机扫码核对。

迁移时须完整恢复资产、二维码身份、打印与贴标记录所在的数据库及附件，正式部署的
`EAM_HOSTNAME` 必须沿用上述域名。只通过 Excel 重建资产会产生新的二维码 Token，旧纸质标签
不能自动关联。贴标后若执行“换标”，旧二维码会按原流程失效。

开发验收时可双击 `启动开发环境-局域网扫码测试.cmd`，临时把独立开发环境发布到当前
局域网。启动窗口会显示手机访问地址；手机必须连接同一网络。该模式使用当前电脑 IP，IP
变化后应重新启动并重新生成/打印测试二维码，不用于正式纸质标签。未配置固定地址时，
使用普通 `启动开发环境.cmd` 会把开发环境恢复为仅 `127.0.0.1:8766` 可访问。

首次启用局域网扫码时，Windows 会弹出管理员确认，用于建立仅允许同一局域网访问 TCP
`8766` 的专用防火墙规则。若 Docker Desktop 因异常退出遗留不可访问的运行时 socket，启动器
会把仅含零字节通信文件的目录改名保留后自动重启 Docker；不会恢复出厂设置，也不会删除镜像、
数据库卷或附件卷。
