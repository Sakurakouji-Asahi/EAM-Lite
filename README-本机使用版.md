...
新打印的资产二维码只包含随机标识，不包含电脑 IP、域名或网址。使用时先在当前 EAM-Lite
登录，再打开顶部“扫码资产”，用页面相机、二维码照片或扫码枪识别；手机系统相机单独扫描
只会显示一串标识。盘点任务的“扫码盘点”页面也能直接识别这类标签。

更换服务器后，完整迁移数据库中资产和二维码身份记录及附件，继续使用新服务器的系统内
扫码入口，已贴的有效标签不因电脑地址改变而重印。仅通过 Excel 重建资产会生成新的二维码
身份，无法自动恢复原标签；执行“换标”后旧标识仍会按原规则失效。早期印有完整网址的标签
可在系统内扫码入口识别，但手机系统相机打开旧网址仍取决于原网址是否可访问。

本机稳定地址 `http://127.0.0.1:8765` 只代表电脑本身，手机无法通过它打开系统。手机
使用网页相机需通过受信任的 HTTPS 地址访问 EAM-Lite；当前局域网 HTTP 验收模式可使用
“二维码照片”或扫码枪。正式多终端使用须另行部署公司 LAN HTTPS 服务。
