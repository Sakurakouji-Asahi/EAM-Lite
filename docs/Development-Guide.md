...
新打印的二维码仅包含 43 字符的高熵随机 Token，不包含任何服务器网址、资产编号或个人
资料。用户先在当前服务器打开受登录保护的“扫码资产”或盘点任务页面，通过页面相机、照片
或扫码枪识别 Token。后端继续按公司、角色、部门/本人范围和标签有效状态鉴权；扫码响应不
缓存，日志遮蔽已进入旧式扫描路径的完整 Token。浏览器相机需要安全上下文；正式移动端
使用受信任的 HTTPS，局域网 HTTP 验收时可用照片或扫码枪。

历史 URL 标签仍可由系统内扫码入口读取路径中的 Token；系统相机若直接打开旧 URL，仍
受其原服务器地址可用性限制。`QR_BASE_URL` 保留供旧 URL 与扫描表单兼容；新标签内容不
再依赖它。服务器迁移必须完整保留数据库中的资产、二维码身份和有效状态以及附件，仅用
Excel 重建会产生新 Token。执行换标会使旧 Token 失效，不能靠改服务器地址复活。
