...
当前实现对应：当前工作入口见 [开发入口](../README-CODEX.md)，Sprint 13–18 已有范围见 [低值物品说明](../README-LOW-VALUE-GOODS.md)。下文保存原路线。

## 1. 执行方式

以下保存最初 Sprint 0–12 的阶段计划；后续 Sprint 13–18 实现与当前工作入口见 [文档索引](README.md)。本节中的阶段检查和等待要求描述原计划，不要求日常维护从 Sprint 0 重做或机械分段。
...
以上是历史里程碑。当前以用户本次任务为范围，不自动启动后续路线，也不要求普通维护逐 Sprint 等待确认。
