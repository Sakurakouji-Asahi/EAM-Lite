...
| UAT-010 | QR label | Clicking print records the batch/operator/time and opens the browser A4 preview without a second print-confirmation step; the preview paginates and renders 500 labels within the Section 3 limit, and at 100% print the QR is at least 20 mm and scans from paper. Authorized asset and label pages show the current non-cacheable QR thumbnail, while scan and Web per-asset attachment confirmation both enforce current identity, printed state, explicit checks, idempotency, audit and field permissions. Edge Android scan and controlled in-app Web confirmation with `Origin: null` (including an app-shell HTTPS Referer) succeed only on their exact endpoints with a Host explicitly listed by `QR_BASE_URL`/non-wildcard `CSRF_TRUSTED_ORIGINS`, valid session/CSRF cookies and form token. Web confirmation additionally requires a short-lived signed bridge bound to the current session and POST path; scan confirmation uses that bridge or, for a pre-bridge page, an exact constant-time URL/form Token match. Missing/tampered Web bridge, missing/mismatched scan Token, missing CSRF, wrong Host, foreign non-null Origin and unrelated POST endpoints remain 403. |
