# 历史原文：README-CODEX.md

> **历史参考，停止作为当前执行指令使用。**
> 2026-09-09 根据用户要求保留修改前原文；原文中的批准、必须、禁止和 Sprint 顺序属于旧设计语境。
> 当前任务按 [根级协作规则](../../AGENTS.md) 和 [文档定位](../README.md) 处理。
...
低值物品扩展以 `docs/13`–`docs/16` 为基线，按
`tasks/Sprint-13-Supplies-Foundation.md` 至 Sprint 18 逐次授权。数量型
物品进入独立 `apps.supplies`；逐件低值耐用品继续使用数量固定为 1 的
现有 Asset，并由财务认定为 `controlled_non_fixed`。不得把扩展解释为
生产物料、采购或通用 ERP 库存授权。
